...
class AdaptiveFFT2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        for period_idx in range(self.num_periods):
            ...
            # Reshape to 2D
            x_2d = x[:, :period * self.top_k, :].view(batch_size, period, self.top_k, d_model)
...
class CrossScaleFusion(nn.Module):
    ...
    def forward(self, scale_features: list[torch.Tensor]) -> torch.Tensor:
        ...
        """
        # Stack and fuse
        stacked = torch.cat(scale_features, dim=-1)  # [batch, seq_len, d_model * num_scales]
...
class TimesBlockPlusPlus(nn.Module):
    ...
    def __init__(
        ...
        self.conv_blocks = nn.ModuleList([
            nn.ModuleList([
                EnhancedInceptionBlock(d_model, d_model, num_kernels=num_kernels)
