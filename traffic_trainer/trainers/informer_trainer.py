"""Informer-based trainer for traffic prediction."""
...
class InformerTrainer(BaseTrainer):
    """Trainer for Informer model."""
    
    config: InformerTrainingConfig
    
    def _load_data(self) -> None:
        """Load dataset for Informer."""
        print(f"Loading dataset for Informer from {self.config.csv_path}...")
        ...
        (
            ...
            graph_mode="none",  # Informer doesn't need graph edges
    ...
    def _get_model_type(self) -> str:
        attn_type = "ProbSparse" if self.config.attn == "prob" else "Full"
        distil_str = " + Distilling" if self.config.distil else ""
        return f"Informer ({attn_type}{distil_str})"
    
    def _get_wandb_config(self) -> Dict[str, Any]:
        """Add Informer-specific info to W&B config."""
    ...
    def _print_model_info(self) -> None:
        """Print model info including Informer-specific details."""
        super()._print_model_info()
        print(f"Segments: {self.num_segments}")
        print(f"Sequence Length: {self.config.seq_len}")
        print(f"Label Length: {self.config.label_len}")
        print(f"Output Length: {self.config.out_len}")
        print(f"Encoder Layers: {self.config.e_layers}")
        print(f"Decoder Layers: {self.config.d_layers}")
        print(f"Attention Heads: {self.config.n_heads}")
        print(f"Attention Type: {self.config.attn}")
        print(f"Distilling: {self.config.distil}")
        print(f"ProbSparse Factor: {self.config.factor}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Informer for traffic prediction")
    parser.add_argument(
        "--config",
        type=Path,
