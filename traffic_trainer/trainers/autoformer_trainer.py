"""Autoformer-based trainer for traffic prediction."""
...
from traffic_trainer.models.autoformer import create_autoformer_model
from traffic_trainer.trainers.base import BaseConfig, BaseTrainer, load_yaml_config


@dataclass
class AutoformerTrainingConfig(BaseConfig):
    """Configuration for Autoformer training."""
    ...
    factor: int = 1  # Auto-correlation factor
    d_ff: int = 2048  # Feed-forward dimension
    n_heads: int = 8  # Number of attention heads
    e_layers: int = 2  # Number of encoder layers
    d_layers: int = 1  # Number of decoder layers
    moving_avg: int = 25  # Moving average window for decomposition
...
class AutoformerTrainer(BaseTrainer):
    ...
    config: AutoformerTrainingConfig
    
    def _load_data(self) -> None:
        """Load dataset for Autoformer."""
        print(f"Loading dataset for Autoformer from {self.config.csv_path}...")
        ...
        (
            ...
            graph_mode="none",  # Autoformer doesn't need graph edges
    ...
    def _create_model(self) -> nn.Module:
        ...
        return create_autoformer_model(
    ...
    def _get_model_type(self) -> str:
        return f"Autoformer (Auto-Correlation + Decomposition)"
    
    def _get_wandb_config(self) -> Dict[str, Any]:
        """Add Autoformer-specific info to W&B config."""
    ...
    def _print_model_info(self) -> None:
        """Print model info including Autoformer-specific details."""
        ...
        print(f"Moving Average Window: {self.config.moving_avg}")
        print(f"Auto-Correlation Factor: {self.config.factor}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Autoformer for traffic prediction")
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).parent.parent / "configs" / "autoformer_config.yaml",
