...
class ARIMATrainer:
    ...
    def _dataset_to_timeseries(self, dataset) -> Dict[int, Dict[str, np.ndarray]]:
        ...
        for seg_id in segment_data:
            segment_data[seg_id]["features"] = np.array(
                segment_data[seg_id]["features"]
            )
    ...
    def _create_arima_model(self, ts: np.ndarray) -> Optional[ARIMA]:
        ...
        try:
            ...
            fitted_model = model.fit(method_kwargs={"maxiter": self.config.max_iter})
            return fitted_model
    ...
    def _predict_arima(self, model: ARIMA, steps: int) -> np.ndarray:
        ...
        try:
            ...
            forecast_int = np.clip(
                np.round(forecast).astype(int), 0, self.num_classes - 1
            )
            return forecast_int
        except Exception:
